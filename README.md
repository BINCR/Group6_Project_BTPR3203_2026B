...
├── rq3_final_ml_predictions.csv        # Machine learning prediction output for RQ3
├── youtube_scrape.py                   # Script to scrape raw YouTube comments
├── data_cleaning.py                    # Script for cleaning, feature engineering, and sentiment labelling
├── analysis.ipynb                      # Main notebook: EDA, statistics, and machine learning
├── requirements.txt                    # Python package dependencies
└── README.md                           # Project documentation
```
...
The first code cell defines the output folder used by all later plotting cells:
...
With this in place, the notebook runs end-to-end from a clean environment via **Restart & Run All** without errors.
...
- `requirements.txt` was generated with `pip freeze > requirements.txt` against the environment used to produce the final results, so package versions match those used for the reported figures (e.g. 63.81% classification accuracy).
- Running the three stages above in order, on a clean environment, reproduces `youtube_comments_with_sentiment.csv`, all three summary CSVs, and all charts in `Figure/` without manual intervention.
